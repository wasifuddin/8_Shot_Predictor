...
def bounce_line(img, pt1, pt2):
    ...
    for i in range(0, 10000):
        i = i / 100
        ePX = int(ref_pt[0] + (xE * i))
        ePY = int(ref_pt[1] + (yE * i))
        if ePX > 155:
            line_ext_err.append([ePX, ePY])


        if wall_touch_check(img, ePX, ePY):
            if inAnyPocket(ePX, ePY, 16):
                prediction = 1
                draw_points_3.append([ePX, ePY, pt2[0], pt2[1]])
                break
            else:

                prediction = 0
                if line_ext_err:
                    line_dis= ((line_ext_err[-1][0] - ePX) ** 2 + (line_ext_err[-1][1] - ePY) ** 2) ** 0.5
